...
public class BatalhaJogadorA implements Initializable {///Fazer Get dos Barcos Do A e Do B
        ...
        for(Barco S : listaDeBarcosA){
            if(S.numDeCasasVivas != 0){
                ...
                while (keys.hasMoreElements()) {
                    ...
                    if(vivo)
                    {
                        System.out.println("Entrou Branco x: " + coordenada.getX() + " y: " + coordenada.getY() + " Morto ou vivo " + S.listaDeDanos.get(coordenada) );
                    ...
                    else{System.out.println("Entrou Vermelho x: " + coordenada.getX() + " y: " + coordenada.getY() + " Tamanho da lista de coordenadas " + S.listaDeDanos.size() );
                        Label label = new Label(S.ID.toString());
    ...
    public void GetAtaquesSofridosA(ArrayList<Point> ataquesSofridosA){
        ...
        for(Point duplo : ataquesSofridosA){
            Label label = new Label("");
    ...
    public void onClickAtacar(){
        FXMLLoader fxmlLoader = new FXMLLoader(HelloApplication.class.getResource("AtacarA.fxml"));
        Scene scene = null;
        try {
        ...
        stage.setTitle("Tabuleiro B Turno A!");
        stage.setScene(scene);

        stage.show();
        AtacarControllerA controllerJanelaA = fxmlLoader.getController();
