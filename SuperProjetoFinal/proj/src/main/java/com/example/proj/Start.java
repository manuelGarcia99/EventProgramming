...
public class Start  implements Initializable {////Se der errado trocar  as coordenadas do gridpane de volta
    ...
    void onClickConfirma(){
        if(estouAEstenderOBarco && passouPelaRodagem){
            ...
            if(CaracteresjaPostos.size() == 5){
                FXMLLoader fxmlLoader = new FXMLLoader(HelloApplication.class.getResource("jogoB.fxml"));
                Scene scene = null;
                try {
                ...
                stage.setTitle("PosicionarB!");
                stage.setScene(scene);

                stage.show();
                StartB controllerJanelaB = fxmlLoader.getController();
                controllerJanelaB.GetListaDeBarcosA(listaDeBarcos);
...
/*System.out.println("LOL");///usar eventos em vez de loops
                    for(Label[] row : matrizDeLabels)//aqui mais tarde criar a classe barco
                    {
                        for(Label l2 : row){
                            if(l2.getStyle().equals(matrizDeLabels[j][i].getStyle()))
                            {
                                l2.setStyle("-fx-border-color: white ; -fx-background-color : darkgreen;");
                            }
                        }
                    }*/
//confirma.onMouseClickedProperty().set(mouseEvent -> {premiConfirma = true;});//temos sempre que voltar atras
//roda.onMouseClickedProperty().set(mouseEvent -> {premiRoda = true;});//temos sempre que voltar atras

//*for (ClickableNode clickableNode : clickables) {
//            clickableNode.node.setOnMouseClicked(event -> {
//                // Handle the mouse click event
//                //clickableNode.setClicked(true);
//
//                clickCounter++;
//                System.out.println("Registei o clique");
//                if(estouAPorUmNavio)
//                semaphore.release();
//                //latch.countDown();
//            });}
...
/*Image barco2 = new Image(currentDirectory+"/ship2.png");
        ImageView navio2 = new ImageView(barco2);
        costa.add(navio2 , 0,2);
        Image barco3 = new Image(currentDirectory+"/ship3.png");
        ImageView navio3 = new ImageView(barco3);
        costa.add(navio3 , 0,3);
        Image barco4 = new Image("file: "+currentDirectory+"/ship4.png");
        ImageView navio4 = new ImageView(barco4);
        costa.add(navio4 , 0,4);
        Image barco5 = new Image( currentDirectory+"/ship5.png");
        ImageView navio5 = new ImageView(barco5);
        costa.add(navio5 , 0,5);*/

 /*  private synchronized void waitForMouseClick() {
        while (!mouseClicked) {
            try {
                wait();
    ...
    public synchronized void notifyMouseClick() {
        mouseClicked = true;
        notifyAll();
    }*/
    /*private void waitForMouseClick(List<ClickableNode> clickableNodes) {
        System.out.println("Entrou na func" + clickCounter );
        latch.countDown();
        try {
            latch.await();
        } catch (InterruptedException e) {
...
/* private List<ClickableNode> createClickableNodes() {
        List<ClickableNode> clickableNodes = new ArrayList<>();
        System.out.println("Criada a lista");



        // Add more buttons or labels as needed

        for(Label[] row : matrizDeLabels)
            for(Label quadrado : row){
                ClickableNode clickableNode = new ClickableNode(quadrado);
                clickableNodes.add(clickableNode);
            }
        for(ImageView imagen : imagens ){
            ClickableNode clickableNode = new ClickableNode(imagen);
            clickableNodes.add(clickableNode);
        }
        for(Label laranjinha : laranjas){
            ClickableNode clickableNode = new ClickableNode(laranjinha);
            clickableNodes.add(clickableNode);
        }
        ClickableNode butaoRodar = new ClickableNode(roda);
        clickableNodes.add(butaoRodar);
        ClickableNode butaoConfirmar = new ClickableNode(confirma);
        clickableNodes.add(butaoConfirmar);
