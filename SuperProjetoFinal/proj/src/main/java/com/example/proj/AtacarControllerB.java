package com.example.proj;////ver linha 172
...
public class AtacarControllerB implements Initializable {
                    ...
                    if(label.getStyle().equals(azul.getStyle())){
                        ...
                        alert.showAndWait().ifPresent(buttonType -> {
                            if (buttonType == okButton) {
                                ///Fazer ataque
                                System.out.println("Debug 1 o ok button funcionou ataque de B a A");
                                for(int r = 0; r<= 13 ; r++){
                                    for(int c =0 ; c<= 13;c++){
                                        if (matrizDeLabels[r][c].equals(label)) {
                                            System.out.println("Na matriz havia o label Ataque de B a A");
                                            ataquesSofridosA.add(new Point(r,c));
                                            for(Barco S : listaDeBarcosA){
                                                Enumeration<Point> keys = S.listaDeDanos.keys();
                                                while (keys.hasMoreElements()){
                                                    Point coordenada = keys.nextElement();
                                                    if(coordenada.getX() == r && coordenada.getY() == c){
                                                        System.out.println("Atingimos!!!!! ataque de B a A X: "+ r +" Y: " + c);
                                                        S.listaDeDanos.put(coordenada, false);
                                                        S.numDeCasasVivas--;
                                                        S.numDeCasasDestruidas++;
                                                        boolean victory = true;
                                                        for(Barco N : listaDeBarcosA){
